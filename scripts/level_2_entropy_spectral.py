# -*- coding: utf-8 -*-
"""
Level 2 - Entropy and Spectral Sensor Selection
-----------------------------------------------

This script identifies an optimal subset of sensors from acceleration time
histories by maximizing a blended utility based on Shannon entropy and spectral
features, while minimizing redundancy based on mutual information and spectral
...
RESULTS_DIR = PROJECT_ROOT / "results" / f"case_study_{CASE_STUDY}" / "level_2"
...
# =============================================================================
# 1. DATA PROCESSING AND SENSOR METRICS
# =============================================================================
...
def compute_metrics_from_data(
    ...
    """
    if not np.isclose(w_entropy + w_spectral, 1.0):
        total_w = w_entropy + w_spectral
        if total_w <= 0:
            raise ValueError("w_entropy + w_spectral must be > 0")
        w_entropy = w_entropy / total_w
        w_spectral = w_spectral / total_w
        warnings.warn("Weights normalized so that w_entropy + w_spectral = 1.")
    ...
    # Blended utility
    b = w_entropy * b_entropy + w_spectral * b_spec
...
def build_qubo_sensor_selection(b: np.ndarray, Corr: np.ndarray, k: int, 
    ...
    if max_corr > 0:
        alpha = alpha / max_corr
        print(f"    [INFO] Dynamic Alpha: scaled alpha to {alpha:.4f} based on max blended redundancy")
...
# =============================================================================
# 4. SOLUTION VALIDATION AND VISUALIZATION
# =============================================================================
def validate_solution(x_opt: np.ndarray, b: np.ndarray, Corr: np.ndarray, 
    ...
    for i, idx_i in enumerate(selected_indices):
        for idx_j in selected_indices[i+1:]:
            correlation_penalty += Corr[idx_i, idx_j]
    
    return utility_score, correlation_penalty, selected_count
...
def print_detailed_results(
    ...
    print(
        f"{'Sensor':<18} "
        f"{'b_total':<10} {'b_entropy':<10} {'b_spec':<10} "
    ...
    for idx in selected_indices:
        ...
        print(
            f"{name:<18} "
            f"{row['b_total']:<10.4f} {row['b_entropy']:<10.4f} {row['b_spec']:<10.4f} "
...
def print_validation_results(utility_score: float, correlation_penalty: float, 
    ...
    print(f"Redundancy penalty:     {correlation_penalty:.4f} (lower is better)")
    if selected_count != k:
...
# =============================================================================
def main() -> None:
    """Run the complete Level 2 sensor-selection workflow."""
    try:
        for directory in (RESULTS_DIR, FIGURES_DIR, TABLES_DIR):
            directory.mkdir(parents=True, exist_ok=True)

        print(f"\n[1] Loading acceleration data from: {ACCELERATION_FILE}")
        df = load_and_prepare_data(ACCELERATION_FILE)
        sensor_names = df.columns.tolist()
        print(f"    Number of available sensor channels: {len(sensor_names)}")

        print("\n[2] Computing entropy and spectral utility metrics...")
        utility, redundancy_matrix, diagnostics = compute_metrics_from_data(
            df=df,
            sample_rate_hz=SAMPLE_RATE_HZ,
            w_entropy=W_ENTROPY,
        ...
        print("\n[4] Solving the QUBO problem with QAOA...")
        solution, optimal_value, objective_history = solve_qubo_qaoa(
            qubo_matrix,
            k=K_SENSORS_TO_SELECT,
            lam=QUBO_LAMBDA,
            reps=QAOA_REPS,
        ...
        print(f"Requested count:      {K_SENSORS_TO_SELECT}")
        ...
        if SAVE_NUMERICAL_RESULTS:
            results = {
                ...
                "sample_rate_hz": SAMPLE_RATE_HZ,
                "available_sensor_count": len(sensor_names),
            ...
            print(f"\n    Selection results saved to: {SELECTION_RESULTS_FILE}")
